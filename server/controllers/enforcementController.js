...
const { getBaseAQI } = require('../config/constants');
...
// GET /api/enforcement/geojson/:city
...
// POST /api/enforcement/dispatch
const dispatchTeam = asyncHandler(async (req, res) => {
...
function generateFallbackRecommendations(city, readings) {
  ...
  const latestAQI = cityReadings[0]?.aqi || getBaseAQI(city);
  ...
  if (hotspots[city]) return hotspots[city];
